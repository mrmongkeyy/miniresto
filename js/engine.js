class Engine{
	...
	initInputs(){
      ...
      this.pause();
		...
		document.addEventListener("visibilitychange", () => {
			if(!this.globalObjects.game.run){
				return;
			}
	...
	toContinueSounds = [];
	stopSoundS(){
		for(let i in this.assets.audio){
			if(!this.assets.audio[i].paused){
				this.toContinueSounds.push(this.assets.audio[i]);
				this.assets.audio[i].pause();
			}
		}
	}
	continueSounds(){
		this.toContinueSounds.forEach(item=>{
			item.play();
		})
		this.toContinueSounds = [];
	}
	pause(){
		this.paused = true;
		this.stopSoundS();
